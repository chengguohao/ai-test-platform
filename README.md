...
> AI 驱动「需求上传 → 接口文档 → 生成用例 → 自动化生成 → 执行报告」全链路，人只保留两次确认：**用例评审认可** 与 **执行结果确认**。
> 环节数据可来自上传 / 粘贴 / URL / MCP / HTTP 等任意接入源；自动化用例采用**声明式生成**——AI 只输出受 JSON Schema 契约约束的用例声明，平台渲染为可执行 pytest 脚本，杜绝 AI「幻觉语法」。
